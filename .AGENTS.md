...
**Use git worktrees for all code work** (unless in a web environment). Never work directly in the main repo checkout for feature/fix branches.

```bash
# Create a worktree branched off dev
git worktree add ../MeshIt-feat-xyz dev
cd ../MeshIt-feat-xyz
git checkout -b feat/xyz

# Symlink .env (never cp — symlinks stay in sync)
ln -s /home/ajb/repos/MeshIt/.env .env
```

- Always use absolute paths when working in worktrees — shell cwd may reset to the main repo after commands
- List existing worktrees: `git worktree list`
- Remove when done: `git worktree remove ../MeshIt-feat-xyz`

### Roadmap Updates

**Regularly update `spec/roadmap.md`** — not just when closing milestones. Update when:

- A new feature is implemented (mark `[x]`, move to "Implemented")
- New features or ideas emerge (add to the appropriate milestone or Backlog)
- New issues are opened for planned features (add issue number to milestone row)
- A milestone's features are all complete (bump `package.json` version)
...
- Never work directly on `main`, almost never directly on `dev` — always use a feature/fix/… branch
  - `dev` is fine for updating specs and documentation
...
- If this work implements a feature, adds a new capability, or opens new issues — update `spec/roadmap.md`
- See [Critical Workflow > Roadmap Updates](#roadmap-updates) for what to update

### 5. Flag browser-testing needs
