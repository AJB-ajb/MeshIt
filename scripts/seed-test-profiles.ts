...
const testProjects: { creatorEmail: string; project: Project }[] = [
  {
    ...
    project: {
      ...
      description: 'Building an intelligent task management app that uses ML to predict completion times and suggest priorities. Looking for frontend developer with React experience.',
  ...
  },
  {
    ...
    project: {
      ...
      description: 'Real-time analytics platform for YouTube and TikTok creators. Need backend developer experienced with data pipelines and APIs.',
...
async function main() {
  ...
  if (existingProfiles && existingProfiles.length > 0) {
    console.log('   Existing profiles:', existingProfiles.map((p: any) => p.email || p.id).join(', '));
  }
  ...
  if (sampleProfile) {
    console.log('   Profile table columns:', Object.keys(sampleProfile).join(', '));
  } else {
    console.log('   No existing profiles - will discover structure during insert');
  }
  ...
  if (sampleProject) {
    console.log('   Project table columns:', Object.keys(sampleProject).join(', '));
  } else {
    console.log('   No existing projects - will discover structure during insert');
  }
  ...
  for (const profile of testProfiles) {
    ...
    if (authError) {
      console.log(`   ⚠️  Auth error for ${profile.email}: ${authError.message}`);
      ...
      if (signInData.user) {
        console.log(`   ✅ User already exists, using existing ID: ${signInData.user.id}`);
  ...
  for (const { creatorEmail, project } of testProjects) {
    ...
    if (projectError) {
      ...
      console.log(`   ✅ Project created: ${project.title} (ID: ${projectData.id})`);
    }
  }
