...
import { createProject, createProjects } from '../utils/factories/project-factory';
...
test.describe('Matching API', () => {
  ...
  test.describe('GET /api/matches', () => {
    ...
    test('sorts matches by similarity score descending', async ({ request }) => {
      const profile = createProfile({
        user_id: userId,
        skills: ['TypeScript', 'React', 'Node.js'],
      });
      ...
      for (let i = 0; i < matches.length - 1; i++) {
        expect(matches[i].similarity_score).toBeGreaterThanOrEqual(matches[i + 1].similarity_score);
      }
    });
  });
  ...
  test.describe('POST /api/matches/:matchId/apply', () => {
    ...
    test('prevents duplicate applications', async ({ request }) => {
      ...
      // Try to apply again
      const secondResponse = await request.post(`/api/matches/${matchId}/apply`, {
        headers: { Authorization: `Bearer ${authToken}` },
      });
