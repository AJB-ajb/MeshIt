...
test.describe('Authentication Flow', () => {
  ...
  test('GitHub OAuth login redirects to GitHub', async ({ page }) => {
    ...
    if (await githubButton.isVisible().catch(() => false)) {
      ...
      test.skip('GitHub login button not found');
    }
  });
  ...
  test('Google OAuth login redirects to Google', async ({ page }) => {
    ...
    if (await googleButton.isVisible().catch(() => false)) {
      ...
      test.skip('Google login button not found');
    }
  });
  ...
  test('Protected routes redirect to login when unauthenticated', async ({ page }) => {
    ...
    for (const route of protectedRoutes) {
      ...
      await expect(page.locator('text=Sign in, text=Login').first()).toBeVisible();
    }
  });
  ...
  test('Auth callback handles successful authentication', async ({ page }) => {
    ...
    const errorMessage = await page.locator('text=error, text=Error, text=failed').first().isVisible().catch(() => false);
  ...
  test('Auth callback handles OAuth errors', async ({ page }) => {
    ...
    await page.goto(`/callback?error=${error}&error_description=${encodeURIComponent(errorDescription)}`);
  ...
  test('API returns user data when authenticated', async ({ request, context }) => {
    ...
    test.skip('Requires valid authentication setup');
  });

  test('Middleware blocks unauthenticated access to dashboard', async ({ page }) => {
  ...
  test('Middleware redirects authenticated users away from login', async ({ page, context }) => {
    ...
    await page.waitForURL('/dashboard', { timeout: 5000, waitUntil: 'networkidle' });
    expect(page.url()).toContain('/dashboard');
  });
});
