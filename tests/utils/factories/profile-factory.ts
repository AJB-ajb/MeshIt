...
const skillPool = [
  ...
  'AWS', 'GCP', 'Azure', 'Next.js', 'Vue.js'
];

const interestPool = [
  'Web Development', 'Mobile Apps', 'AI/ML', 'DevOps',
  'Game Development', 'Open Source', 'Startups',
  'Education', 'Healthcare Tech', 'FinTech'
];

export const createProfile = (overrides: Partial<TestProfile> = {}): TestProfile => {
  const skills = faker.helpers.arrayElements(skillPool, faker.number.int({ min: 3, max: 7 }));
  const interests = faker.helpers.arrayElements(interestPool, faker.number.int({ min: 2, max: 5 }));
  ...
  return {
    ...
    collaboration_style: faker.helpers.arrayElement(['sync', 'async', 'flexible']),
...
export const createProfiles = (count: number, overrides: Partial<TestProfile> = {}): TestProfile[] => {
 ...
 */
export const createBeginnerProfile = (overrides: Partial<TestProfile> = {}): TestProfile => {
 ...
 */
export const createAdvancedProfile = (overrides: Partial<TestProfile> = {}): TestProfile => {
  return createProfile({
